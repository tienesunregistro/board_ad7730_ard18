...
def main() -> int:
    ...
    parser.add_argument("--boot-wait", type=float, default=2.5, help="Espera tras abrir puerto (reset Arduino), segundos")
    ...
    try:
        with serial.Serial(args.port, args.baud, timeout=args.timeout, write_timeout=1) as ser:
            ...
            if args.boot_wait > 0:
                print(f"Esperando arranque placa: {args.boot_wait:.1f}s")
